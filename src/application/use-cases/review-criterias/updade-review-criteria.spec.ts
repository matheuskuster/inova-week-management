...
describe('Update Review Criteria', () => {
    ...
    it('should throw an error if review criteria does not exist', async () => {
        await expect(updateReviewCriteria.execute({ id: 'invalid-id' })).rejects.toThrow();
    });
    ...
    it('should update the review criteria if it exists', async () => {
        ...
        await updateReviewCriteria.execute({ id: reviewCriteria.id, question: 'new-question' });

        const updatedReviewCriteria = await reviewCriteriaRepository.findById(reviewCriteria.id);
